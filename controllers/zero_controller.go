	...
	pod := &corev1.Pod{
		...
		Spec: corev1.PodSpec{
			...
			Containers: []corev1.Container{{
				...
				Name:           name,
				Env:            PodEnv(ispn, &[]corev1.EnvVar{{Name: "IDENTITIES_BATCH", Value: OperatorConfMountPath + "/" + consts.ServerIdentitiesCliFilename}}),
			...
			Volumes: []corev1.Volume{
				...
				// EmptyDir for Infinispan data volume as the zero-capacity node does not store traditional data
				{
					...
					VolumeSource: corev1.VolumeSource{
						Secret: &corev1.SecretVolumeSource{
							SecretName: ispn.GetInfinispanXmlConfigName(),
						},
					},
				},
			},
